...
$.widget('jui.splitText', {
    ...
    _create: function () {
        ...
        ops.originalText = self.element.text()
            .replace(/\s{3,}/gim, '  ');

        // Split
        self.toggleSplitText();

        self.element.trigger(ops.eventsPrefix + ':complete');
    },
    ...
    toggleSplitText: function (state) {
        ...
        if (state === ops.states.unsplit) {
            ...
            self.element.html(self._splitStrToSpans(ops.originalText));
        }
        else {
            ops.state = ops.states.unsplit;
            self.element.text(ops.originalText);
        }
